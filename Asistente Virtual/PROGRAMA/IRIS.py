...
#Listado de pregunstas y respuestas para el chat bot
bot = list()
sql = sqlite3.connect("lib/brain.db")
cursor = sql.execute('''SELECT * FROM question_answer''')
filas = cursor.fetchall()
for fila in filas:
    bot.extend(list(fila))
chat = ChatBot("iris", database_uri=None)
...
#Función que ofrece una respuesta predeterminada a una pregunta (ChatBot)
def getDefaultAnswer(rec):
